...
$tags = get_the_terms( $_GET['id'] , 'product_tag' );
...
$current_collection = $wpdb->get_results("SELECT * FROM $table WHERE product_id = ".$_GET['id']."", ARRAY_A);


$all_collection = "";
foreach($current_collection as $collection){
    $all_collection .= $collection['collection_name'].",";
   

}
echo "<script>  var all_collection = '".$all_collection."'; </script>";
$start_date;
if(isset($product->date_on_sale_from))
{
    $start_date = $product->date_on_sale_from;
...
<body>
        ...
        <div id="content">
          ...
          <h1 class="dashboard-title primary-color1 padding-15px"><a href="homepage.php"><i class="flaticon-back"></i></a>  ADD PRODUCT</h1>
                ...
                <div class="row no-margin">
                    ...
                    <form action="woocommerce_api/edit_product.php" method=POST enctype=multipart/form-data id="product">
                        <div class="backend-course-image" >
                            <img src = "<?php echo get_the_post_thumbnail_url($_GET['id']); ?>">
                        </div>
                        ...
                        <div class="drop-shadow card-holder row no-margin no-padding" id = "secondary-image">
                            <div class="col-xl-6 col-12 backend-course-image pip">
                                <?php foreach( $attachmentIds as $attachmentId ): ?>
                                <img src="<?php echo wp_get_attachment_url( $attachmentId ); ?>">
                                <div id="remove">
                                    <input id="valueImage" type="hidden" value="<?php echo $attachmentId;?>">
                                    <h6 class="dashboard-title3-bold secondary-color1 text-center remove" style="margin-top:5px;">
                                        REMOVE
                                        <i class="flaticon-rubbish-bin flt secondary-color1" style="padding-left:10px;padding-right:10px;margin-bottom:10px;cursor:pointer;"></i>
                                    </h6>
                                </div> 
                                <?php endforeach; ?>
                            </div>
                        ...
                        <div class="drop-shadow card-holder">
                            ...
                            <div id="category" class="collapse collapsible-div" >
                                ...
                                <select name = "category" class="input-lg form-control">
                                    <option value = "<?php echo $product->categories[0]->name; ?>"><?php echo $product->categories[0]->name; ?></option>
                                    <?php foreach($categories as $category):?>
                                    <?php if($product->categories[0]->name == $category->name) continue;?>
                                        <option value = "<?php echo $category->id ;?>"><?php echo $category->name ;?></option>
                                    <?endforeach;?>   
                                </select>
                                <i class="flt flaticon-back rotated-90 grey"></i>
                                <h6 class="dashboard-title3 primary-color1">Tags</h6>
                                <input type="text" data-role="tagsinput" class="form-control" id="tag" name="tags" value ="<?php echo $tag_string; ?>">
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="col-xl-6 col-lg-6">
                      <div class="drop-shadow card-holder">
                            <div class="collapsible-div-button" type="button" >
                                ...
                                <p class="primary-color1 dashboard-title3 ">Start Date: </p><input id="startDate" style="width:80%;max-width:300px;margin-bottom:10px;" placeholder="<?php if(!empty($product->date_on_sale_from))echo $start_date; else echo "MM/DD/YYYY"; ?>" class="primary-color1"/>
                                <p class="primary-color1 dashboard-title3 ">End Date: </p><input id="endDate" style="width:80%;max-width:300px;margin-bottom:10px;" placeholder="<?php if(!empty($product->date_on_sale_to))echo $end_date; else echo "MM/DD/YYYY"; ?>" class="primary-color1"/>
                                <input type="hidden" name="end_date" id="endval" value="<?php  if(!empty($product->date_on_sale_from))echo $start_date; ?>"/>
                                <input type="hidden" name="start_date" id="startval" value="<?php if(!empty($product->date_on_sale_to))echo $end_date; ?>"/>
                            </div>
                            ...
                            <div id="category" class="" style="padding:15px">
                                ...
                                <p class="primary-color1">Used to calculate shipping rates at checkout and label prices during fulfilment</p>
                                 <div class="form-group">
                                    ...
                                    <input type="number" class="form-control" name = "weight" value = "<?php echo $product->weight; ?>">
                                </div>
            ...
            <script>
                var html = "<?php echo $product->description;?>";
    ...
    var array_2 = all_collection.split(",");
    ...
    //console.log(select);
     $(function () {
        ...
        $('#select').multipleSelect('setSelects', array_2);
      })
    </script>
    ...
    <script>
        ...
        {
            var image_id = $(this).children('#valueImage').val();
            var id = <?php echo $_GET['id']; ?>;
            var sendData = {"image_id" : image_id , "id" : id};
            console.log(sendData);
            $.ajax({
        		url : "ajax_call/remove_product_image.php",
        		data : sendData,
        		type : "POST",

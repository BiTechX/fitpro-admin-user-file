...
$result_2 = $wpdb->get_results ("
   SELECT * FROM `$tableName` WHERE `plan_is_featured` = 1
");
$featured_plan = '0';
if(count($result_2) >= 1){
    $featured_plan = $result_2[0]->Id;
}
//
//print_r($result);
    ...
    <div class="wrapper">
        ...
        <div id="content" style="padding-top:0px;">
                ...
                <div class="row">
                    ...
                    <div class="col-xl-12 module-card-div" style="padding-top:20px;"> <!-- padding top 20px more for shadow -->
                       ...
                       <div class="padding-15px" style="margin-bottom:20px;display:inline-block;">
                           <h1 class="dashboard-title primary-color1" style="margin-bottom:5px;"><a href="homepage.php"><i class="flt flaticon-back primary-color1" style="margin-right:20px;vertical-align:top;"></i></a>set pricing</h1>
                           <p class="small-text dashboard-title3-bold secondary-color1" style="margin-left:60px;">Add upto three payment options</p>
                       </div>
                        <div class="row  no-margin" style="margin-bottom:30px;">
                            <div class="col-xl-4">              
                                <p class="small-text dashboard-title3-bold primary-color1" >SET FEATURED PRICE PLAN</p>
                                <select class="input-lg form-control" style="margin-bottom:0px;"  id="featured_plan_id">
                                    
                                    <option value="0">NONE</option>
                                    
                                    <?php foreach($result as $val): ?>  
                                     <option value="<?php echo $val->Id; ?>"><?php echo $val->plan_title; ?></option>
                                    <?php endforeach ?>            
                            ...
                            <div class="col-xl-4">              
                                ...
                                <div class="row vertical-middle no-margin" style="max-width:500px;margin-top:20px;">  <!-- bottom-buttons-->
                                      
                                     <div class="col-12 no-padding text-center text-sm-left">
                                          <button class="horizontal-middle drop-shadow btn theme-rounded-button white secondary-color1-background secondary-color1-border secondary-color1-border-hover secondary-color1-hover 
                                           transparent-background-hover margin-bottom-10px-mobile" type="submit" id="ActionButton_featured" >SAVE</button>
                                    </div>
                                </div>
                            </div>                          
                        </div>
                        ...
                        <?php endforeach ?>  
                                <div class="col-xl-4 col-sm-6 col-12">
                                     <a href="<?php echo site_url().'/admin/set_pricing_settings.php' ?>">
...
<script>
       $(document).ready(function(){
                ...
                var value = '<?php echo $featured_plan;?>';
                $('option[value='+value+']').attr('selected', 'selected');
                
           
    });
    
    $(document).on('click', "#ActionButton_featured", function(e) {
      
                var postvalue =  jQuery('#featured_plan_id').val();
                var url = "<?php echo FITPRO_THEME_BTX_fun_new_plan_add_featured_admin_url().'/';?>"+postvalue;
                ...
                $.ajax({
                    ...
                    beforeSend : function()
                    {
                        jQuery("#ActionButton_featured").prop("disabled",true);
                        jQuery("#ActionButton2").prop("disabled",true);
                    },
                    success: function(result){
                        console.log(result);
                       jQuery("#ActionButton_featured").prop("disabled",false);

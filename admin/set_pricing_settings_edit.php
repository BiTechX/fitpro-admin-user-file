...
if(!isset($_GET['edit_id'])  ){
    wp_redirect( site_url()."/admin/set_pricing.php" ); exit; 
}
...
$result = $wpdb->get_results ( "
    SELECT * FROM `".plan_db_info()."` 
    INNER JOIN `".plan_type_db_info()."` 
    ON `".plan_db_info()."`.`plan_type` = `".plan_type_db_info()."`.`ID` 
    WHERE `".plan_db_info()."`.`Id` = $id
");
                        ...
                        <div class="drop-shadow-left white-background padding-15px scrollbar-change secondary-color1-scrollbar mobile-overlapping-sidebar" style="padding-top:20px;height:100vh;overflow:auto;"> <!-- padding top 20px more for shadow -->
                            <div class="row vertical-middle" style="margin:0 auto 20px;max-width:500px;">
                                <div class="col-12 no-padding">
                                    ...
                                    <select class="input-lg form-control" style="margin-bottom:0px;" id="plan_type" disabled>
...
<script>
    ...
    $(document).ready(function(){
                ...
                var value = '<?php echo $result[0]->plane_type;?>';
                 $('option[value='+value+']').attr('selected', 'selected');
                if(value == 'ONE_TIME_PAYMENT'){
                    $('#ALL_PAYMENT').html("");
                     $('#ALL_PAYMENT').load("<?php echo site_url().'/admin/ajax_call/one_time_member_form.php?edit_id='.$_GET['edit_id'] ; ?>");
                }
                else if(value == 'MONTHLY'){
                      $('#ALL_PAYMENT').html("");
                      $('#ALL_PAYMENT').load("<?php echo site_url().'/admin/ajax_call/monthly_member_form.php?edit_id='.$_GET['edit_id'] ; ?>");
                }
                else if(value == 'INSTALMENT'){
                    $('#ALL_PAYMENT').html("");
                    $('#ALL_PAYMENT').load("<?php echo site_url().'/admin/ajax_call/instalment_member_form.php?edit_id='.$_GET['edit_id'] ; ?>");
                }
                else if(value == 'FREE'){
                     $('#ALL_PAYMENT').html("");
                     $('#ALL_PAYMENT').load("<?php echo site_url().'/admin/ajax_call/free_member_form.php?edit_id='.$_GET['edit_id'] ; ?>");
                }
    ...
    $(document).on('submit', "#membership_option_set", function(e) {
                ...
                var id_url =  "<?php echo $id ; ?>";
                var postvalue =  jQuery(this).serialize();
                var url = "<?php echo FITPRO_THEME_BTX_fun_new_plan_edit_admin_url();?>/"+id_url;
